      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 3 }}>
            {[
              ["Cal", meal.cal, theme.red, "kcal"],
              ["Prot", meal.protein, theme.blue, "g"],
              ["Carbs", meal.carbs, theme.yellow, "g"],
              ["Fat", meal.fat, theme.green, "g"],
            ].map(([l, v, c, u]) => (
          ...
          <div style={{ display: "flex", gap: 2, marginTop: 4, height: 3 }}>
            <div style={{ flex: calPct, background: theme.red, borderRadius: "2px 0 0 2px", opacity: 0.5 }} />
            <div style={{ flex: proteinPct, background: theme.blue, opacity: 0.5 }} />
            <div style={{ flex: carbsPct, background: theme.yellow, opacity: 0.5 }} />
            <div style={{ flex: fatPct, background: theme.green, borderRadius: "0 2px 2px 0", opacity: 0.5 }} />
          </div>
        </div>
        <motion.button
          whileHover={{ scale: 1.15, color: theme.red }}
          whileTap={{ scale: 0.9 }}
          onClick={(e) => { e.stopPropagation(); handleDelete(); }}
          style={{
            background: "transparent", border: "none",
            color: theme.textDim, cursor: "pointer", padding: 4, display: "flex", flexShrink: 0,
          }}
...
function MealSection({ time, items, secCal, secProt, secCarbs, secFat, totalCal, onOpenModal, onDeleteMeal }) {
  ...
  const color = MEAL_COLORS[time] || theme.red;
      ...
      <div style={{
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 20 }}>{MEAL_ICONS[time]}</span>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: theme.text, display: "flex", alignItems: "center", gap: 6 }}>
                {time}
                {!isEmpty && (
                  <span style={{
                    fontSize: 10, fontWeight: 500, color: theme.textMuted,
                    background: theme.bgCard3, padding: "1px 7px", borderRadius: radius.full,
                  }}>
                    {items.length}
                  </span>
                )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {!isEmpty && (
              <div style={{ fontSize: 11, color: theme.textMuted, fontWeight: 500, fontVariantNumeric: "tabular-nums" }}>
                {pct}%
              </div>
            )}
              ...
              <div style={{ padding: "0 16px 12px", borderTop: `1px solid ${theme.border}` }}>
                {isEmpty ? (
                  ...
                  <>
                    ...
                    <motion.button
                      whileHover={{ borderColor: theme.red + "30" }}
                      whileTap={{ scale: 0.98 }}
                      ...
                      style={{
                        ...
                        color: theme.textMuted, borderRadius: radius.md,
...
function generateInsights(totalCal, totalProt, totalCarbs, totalFat, totalFiber, totalSugar, water, meals, calGoal, protGoal) {
  const tips = [];
  if (totalProt < protGoal * 0.7) tips.push({ icon: "🥩", text: `Protein is low. Need ${Math.round(protGoal - totalProt)}g more.`, color: theme.red, priority: 1 });
  else if (totalProt >= protGoal) tips.push({ icon: "💪", text: "Protein goal achieved! Great work.", color: theme.green, priority: 1 });
  if (totalFiber < 25) tips.push({ icon: "🥦", text: `Fiber is low (${Math.round(totalFiber)}g). Aim for 25-30g daily.`, color: theme.yellow, priority: 2 });
  if (totalSugar > 50) tips.push({ icon: "🍬", text: `Sugar is high (${Math.round(totalSugar)}g). Try to stay under 50g.`, color: theme.orange, priority: 2 });
  if (water < 5) tips.push({ icon: "💧", text: `Hydration needs work. ${Math.round((8 - water) * 250)}ml remaining.`, color: theme.blue, priority: 3 });
  else if (water >= 8) tips.push({ icon: "💧", text: "Excellent hydration! Keep it up.", color: theme.teal, priority: 3 });
  if (meals.length < 3) tips.push({ icon: "⏰", text: "Only 2 meals logged. Try to eat 3-4 balanced meals.", color: theme.purple, priority: 4 });
  else if (meals.length >= 4) tips.push({ icon: "🎯", text: "Great meal frequency! Consistent eating pattern.", color: theme.green, priority: 4 });
  if (totalCal < calGoal * 0.8) tips.push({ icon: "🔥", text: `Calories are low. ${Math.round(calGoal - totalCal)} kcal remaining.`, color: theme.orange, priority: 4 });
  else if (totalCal > calGoal * 1.1) tips.push({ icon: "⚡", text: `Calories exceeded by ${Math.round(totalCal - calGoal)} kcal today.`, color: theme.red, priority: 4 });
            ...
            >
              ...
              {active && (
                <motion.div layoutId="nutrition-tab" transition={{ type: "spring", stiffness: 500, damping: 35 }}
                  style={{ position: "absolute", bottom: 0, left: 8, right: 8, height: 2, background: theme.red, borderRadius: 1, boxShadow: `0 0 6px ${theme.red}60` }} />
              )}
            </motion.button>
          );
        })}
      ...
      <svg width="0" height="0" style={{ position: "absolute" }}>
        <defs>
          <linearGradient id="calGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor={theme.red} /><stop offset="100%" stopColor={theme.redLight} />
          </linearGradient>
          <linearGradient id="proteinGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor={theme.blue} /><stop offset="100%" stopColor="#60a5fa" />
          </linearGradient>
          <linearGradient id="carbsGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor={theme.yellow} /><stop offset="100%" stopColor="#fbbf24" />
          </linearGradient>
          <linearGradient id="fatGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor={theme.green} /><stop offset="100%" stopColor="#4ade80" />
          </linearGradient>
          <linearGradient id="fiberGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor={theme.teal} /><stop offset="100%" stopColor="#2dd4bf" />
          </linearGradient>
        </defs>
      </svg>
    </motion.div>
